...
class VideoDescriptionRealTime(object):
    ...
    def test(self):
        ...
        if(filename == "NA"):
            return "End of files", "No more files"
    ...
    def evaluate_model(self, caption, predicted_caption):
        """
        Evaluate the model using BLEU and METEOR scores on the validation set
        """
        ...
        bleu = sentence_bleu(reference, candidate, weights=(0.25, 0.25, 0.25, 0.25), smoothing_function = smoothing_function) # Using BLEU-4
...
if __name__ == "__main__":
    ...
    captions = [
        ...
        "a cat is dancing", 
        "a woman is preparing a sandwich", 
        "a man is riding a bike", 
        "a man is preparing food", 
        "a man is becoming a woman",
        "a man is playing a guitar",
        "a person is puring rice in a bowl",
        "a man is adding something in a pan"
        ]
    bleu_scores = []
    meteor_scores = []
    ...
    while True:
        ...
        if(video_caption == "End of files"):
            break
        end = time.time()
        sentence = ''
        print(sentence)

        bleu, meteor = video_to_text.evaluate_model(captions[count], video_caption)
        ...
        if play_video.lower() == 'y':
            ...
            continue
    print(f"Average BLEU-4 score: {np.mean(bleu_scores)}")
