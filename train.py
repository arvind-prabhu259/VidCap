...
class VideoDescriptionTrain(object):
    ...
    def evaluate_model(self, validation_list):
        """
        ...
        for entry in validation_list:
            ...
            predicted_caption = self.predict(video_id)  # Assuming self.predict method exists for inference

            # BLEU score
            reference = [caption.split()]
            candidate = predicted_caption.split()
            bleu = sentence_bleu(reference, candidate, weights=(0.25, 0.25, 0.25, 0.25), smoothing_function = SmoothingFunction().method1) # Using BLEU-4
        ...
        print(f"Average BLEU-4 Score: {avg_bleu:.3f}")
...
if __name__ == "__main__":
    ...
    video_to_text.evaluate_model(validation_list)
